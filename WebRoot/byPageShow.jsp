      ...
      if(totalPages>=1) {
         ...
         for(int i=1;i<=pageSize&&boo;i++) { 
            ...
            String button="<form  action='putGoodsServlet' method = 'post'>"+
                     ...
                     "<input type ='submit' class='btn btn-sm btn-success'  value='放入购物车' ></form>";
            String detail="<form  action='showDetail.jsp' method = 'post'>"+
                     "<input type ='hidden' name='xijie' value= "+number+">"+
                     "<input type ='submit' class='btn btn-sm btn-info' value='查看细节' ></form>";
            out.print("<tr>");
...
<Table>
  <tr><td><FORM action="" method=post>
	          ...
	          <button type="submit" class="btn btn-sm btn-default">上一页</button>
          </FORM></td>
      <td><FORM action="" method=post>
	          <Input type=hidden name="currentPage"
	           value="<%=dataBean.getCurrentPage()+1 %>">
	           <button type="submit" class="btn btn-sm btn-default">下一页</button>
	      </FORM></td></tr>
 <tr><td> <FORM action="" method=post>
          每页显示<Input type=text name="pageSize" value =2 size=3>
          条记录<button type="submit" class="btn btn-sm btn-default">确定</button></FORM></td>
      <td> <FORM action="" method=post>
